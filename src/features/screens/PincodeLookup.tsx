...
import { FieldMessage, Text } from "@/ui";
...
export function PincodeLookup({ onResolved }: { onResolved: (resolution: PincodeResolution) => void }) {
  ...
  return (
    <View style={styles.wrapper}>
      ...
      <View style={[styles.row, { borderColor: status === "not-found" ? colors.danger : colors.border, backgroundColor: colors.card }]}>
        <TextInput
          value={pincode}
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: "row", alignItems: "center", minHeight: 48, borderWidth: 1, borderRadius: radii.md, paddingHorizontal: spacing(3), gap: spacing(2) },
